...
import { TitleData } from './DashboardScreen';
...
class HistoryScreen extends React.Component<Props, State> {
  ...
  generatorAvatar() {
    ...
    return (
      <Svg width={20} height={20} viewBox="10 -20 100 100">
        <G rotate="0" scale="1 1" fill="#34C8FF">
  ...
  renderBatonTitle = (title: string, i: number) => (
    <ListItem
  ...
  renderBatTitle = (title: string, i: number) => (
    <ListItem
        ...
        db.collection(`users/${currentUser.uid}/todayTitle`)
          ...
          .then(querySnapshot => {
            querySnapshot.forEach(doc => {
              fbData.push(doc.data() as TitleData);
            });
            if (fbData.length > 0) {
              const noSaved =
                data.length > 0
                  ? fbData.filter((fb: TitleData) =>
                      data.every((d: TitleData) => fb.date !== d.date)
                    )
                  : [];
  ...
  renderListItem(item: TitleData, i: number) {
